...
const deleteCommandMessages = function (msg, client) { // eslint-disable-line consistent-return
    if (msg.deletable && client.provider.cache.get('global', 'deletecommandmessages', false)) {
      return msg.delete();
    }
  };
...
class Util {
	...
	static async randomFromImgurAlbum(album) {
        const { body } = await request
            .cache.get(`https://api.imgur.com/3/album/${album}`)
    ...
    static today(timeZone) {
        const now = new Date();
        if (timeZone) now.setUTCHours(now.cache.getUTCHours() + timeZone);
        now.setHours(0);
        now.setMinutes(0);
        now.setSeconds(0);
	...
	static tomorrow(timeZone) {
        ...
        today.setDate(today.cache.getDate() + 1);
        return today;
    }
