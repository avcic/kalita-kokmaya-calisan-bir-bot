...
exports.run = async (client, message, params) => {  
    ...
    var konum = 'Konumlar'
       ...
       if(moment(message.guild.createdAt).format('MM') === '06') {
        ...
        .setColor("333333")
        .addField(`|> Kullanıcılar [${message.guild.memberCount}]`, `${message.guild.members.cache.filter(m => m.user.presence.status === "online").size} Aktif \n ${message.guild.members.cache.filter(m => m.user.presence.status === "idle").size} Boşta \n  ${message.guild.members.cache.filter(m => m.user.presence.status === "dnd").size} Rahatsız Etmeyin \n ${message.guild.members.cache.filter(m => m.user.presence.status === "offline").size} Çevrim Dışı \n   ${message.guild.members.cache.filter(m => m.user.bot).size} Bot`, true)
        .addField(`|> Kanallar`, `Toplam Kanal Sayısı : ${message.guild.channels.size} \nToplam Yazı Kanalı : ${message.guild.channels.cache.filter(c => c.type === "text").size} \nToplam Ses Kanalı : ${message.guild.channels.cache.filter(c => c.type === "voice").size}`, true)
