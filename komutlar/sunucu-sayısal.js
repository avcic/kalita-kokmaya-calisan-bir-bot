exports.run = async(client, message, args) => {
    ...
    **${message.guild.members.cache.filter(a => !a.bot).size}** Kullanıcı
    **${message.guild.members.cache.filter(a => a.bot).size}** Bot
    Çevrimdışı Üyeler **•** **${message.guild.members.cache.filter(a => a.presence.status == "offline").size}**
    Çevrimiçi Üyeler **•** **${message.guild.members.cache.filter(a => a.presence.status != "offline").size}**
