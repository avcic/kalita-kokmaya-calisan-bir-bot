                    ...
                    emojis.forEach(async emoji => {
                        const timeout = setTimeout(async () => {
                            ...
                            if (kalan == 0) {
                                ...
                                file.addLocalFolder('./emojiler', 'Güllü Emoji Ara');
                                fs.writeFileSync('/gullu.zip', file.toBuffer());
                                ...
                                message.channel.send({
                                    files: [{ name: 'gullu.zip', attachment: '/gullu.zip' }]
                                }).then(() => {
                                    fs.promises.rmdir('emojiler', { recursive: true }).then(() => console.log("emojiler klasorunu sildim, Ghost Bey"))
                                });
                            }
                        }, yuklenen * 2000);
                        yuklenen = yuklenen + 1;
